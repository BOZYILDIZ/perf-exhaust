...
import { Search, PencilLine, Trash2, Loader2 } from "lucide-react";
...
export default function FAQTable({ initialRows }: { initialRows: AdminFAQRow[] }) {
  ...
  const remove = async (row: AdminFAQRow) => {
    if (!window.confirm(`Supprimer définitivement « ${row.question} » ?\nCette action est irréversible.`)) return;
    setDeleting(row.id);
    setMessage(null);
    try {
      ...
      setMessage({ type: "ok", text: "Question supprimée." });
      router.refresh();
    } catch (e) {
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 text-xs mb-3">{filtered.length} question{filtered.length > 1 ? "s" : ""}</p>
            ...
            {filtered.map((r, i) => (
              <tr key={r.id} style={{ background: i % 2 ? "#0d0d0d" : "#0f0f0f", borderTop: "1px solid #1a1a1a" }}>
                <td className="px-4 py-3">
                  <div className="text-white font-medium max-w-md truncate">{r.question}</div>
                  {r.category && <div className="text-gray-600 text-xs">{r.category}</div>}
                </td>
